...
exports.SIMPLEDOM_CHILDREN = undefined;

var _typeof = typeof Symbol === "function" && typeof Symbol.iterator === "symbol" ? function (obj) { return typeof obj; } : function (obj) { return obj && typeof Symbol === "function" && obj.constructor === Symbol && obj !== Symbol.prototype ? "symbol" : typeof obj; };

var _extends = Object.assign || function (target) { for (var i = 1; i < arguments.length; i++) { var source = arguments[i]; for (var key in source) { if (Object.prototype.hasOwnProperty.call(source, key)) { target[key] = source[key]; } } } return target; };

exports.setSimpleDomChildren = setSimpleDomChildren;
exports.el = el;
...
var i = 0;
var SIMPLEDOM_CHILDREN = exports.SIMPLEDOM_CHILDREN = [];
function setSimpleDomChildren(newChildren) {
    exports.SIMPLEDOM_CHILDREN = SIMPLEDOM_CHILDREN = newChildren;
}

var isReactComp = function isReactComp(elem) {
    return elem && elem.prototype && elem.prototype.__proto__.isReactComponent;
};

function buildObject(element) {
    //, simpleDomChildren = []) { //, counter = 0) {
    if (element.isReactComponent) {
        return _react2.default.createElement(element.componentClass, element.attrs, element.children);
    } else if (element.isComponent) {
        var newElem = _react2.default.createElement('div', { 'data-attribute': i }, []);
        SIMPLEDOM_CHILDREN.push({ id: i, element: element });
        ++i;
        return newElem;
    ...
    return _react2.default.createElement(element.name, element.attrs, (element.children || []).filter(function (c) {
...
function el(element, attrs) {
    ...
    if (element && element.isComponent) {
        ...
        const allChildren = flatten(children).map(child => child.simpleDomChildren || []);
         const simpleDomChildren = flatten(allChildren)
            .concat(children)
        ...
        */
        ...
        Object.keys(attrs).forEach(function (keyAttr) {
            var param = attrs[keyAttr];
            console.log(param);
            if (param.isComponent || param.isElem || isReactComp(param)) {
                attributes[keyAttr] = buildObject(param); //, simpleDomChildren); //, i);
                return;
            }
            attributes[keyAttr] = param;
        });
        // console.log('simpleDomChildren of react Comp: ', simpleDomChildren)
        return {
...
function renderToDom(node, component) {
    ...
    var event = new Event('beforerender');
